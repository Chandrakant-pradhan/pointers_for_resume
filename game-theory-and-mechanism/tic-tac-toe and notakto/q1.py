...
class History:
    ...
    def is_draw(self):
        ...
        return flag and not self.is_win()
...
def backward_induction(history_obj):
    ...
    # 0}}
    # (2b) Note, the dici for each history in strategy_dict_x and strategy_dict_o is probability distribution over
    # actions. But since tictactoe is a PIEFG, there always exists an optimal deterministic dici (SPNE). So your
    ...
    dici = {str(i): 0 for i in range(9)}
    if act is not None:
        dici[str(act)] = 1

    if history_obj.player == 'x':
        strategy_dict_x[history_key] = dici
    else:
        strategy_dict_o[history_key] = dici
