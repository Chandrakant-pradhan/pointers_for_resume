...
class History:
    ...
    def is_terminal_history(self):
        # Feel free to implement this in anyway if needed
        empty_space = self.get_valid_actions()
        return (empty_space == 0) or (self.is_win())
...
def custom_sort(action):
    if action % 9 == 4:
        return (1, action % 9 in [0, 2, 6, 8])  # Center squares first, then corners
    else:
        return (0, False)
...
def alpha_beta_pruning(history_obj, alpha, beta, max_player_flag):
    ...
    #valid_actions.sort(key=custom_sort , reverse=True)
    valid_actions.sort(key=lambda x: (x % 9 == 4, x % 9 in [0, 2, 6, 8]), reverse=True)
    if max_player_flag:
        max_val = -math.inf
        for action in valid_actions:
            next_hist = copy.deepcopy(history_obj)
            next_hist.update_history(action)
            max_val = max(max_val, alpha_beta_pruning(next_hist, alpha, beta, False))
        ...
        for action in valid_actions:
            next_hist = copy.deepcopy(history_obj)
            next_hist.update_history(action)
            min_val = min(min_val, alpha_beta_pruning(next_hist, alpha, beta, True))
...
def maxmin(history_obj, max_player_flag):
    ...
    if max_player_flag:
        ...
        for action in valid_actions:
            next_hist = copy.deepcopy(history_obj)
            next_hist.update_history(action)
            temp = next_hist.get_boards_str()
            if temp not in board_positions_val_dict:
                board_positions_val_dict[temp] = maxmin(next_hist, False)
            max_val = max(max_val, board_positions_val_dict[temp])
        return max_val
    else:
        min_val = math.inf
        valid_actions = history_obj.get_valid_actions()
        for action in valid_actions:
            ...
            temp = next_hist.get_boards_str()
            if temp not in board_positions_val_dict:
                board_positions_val_dict[temp] = maxmin(next_hist, True)
            min_val = min(min_val, board_positions_val_dict[temp])
        return min_val
    # TODO implement
