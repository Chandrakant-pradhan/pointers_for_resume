...
def make_util_matrix(num_players, strategy, util_list):
    ...
    strategy (list): A list containing the number of strategies each target has.
...
def sel_index(target, args, multiplier, num_players):

    """
    Selects the index in the game data based on the target and their chosen strategies.

    Args:
    target (int): The target for whom the index is calculated.
...
def find_strongly_dominant_eq(gamedata, playerno, totalplayer, topplayer, strategies,
    ...
    """
    Finds strongly dominant strategy equilibrium for a given target.

    Args:
    gamedata (list): The payoff matrix in Gambit nfg format 
    playerno (int): The index of the target for whom the equilibrium strategy is sought.
    totalplayer (list): A list of remaining players to consider.
    topplayer (int): The index of the top target in the list of remaining players.
    strategies (list): List containing the number of strategies of each target.
    multiplier (list): Cumulative product of the "strategies" list, used to find the index of
                       a target's utility within a strategy (using the sel_index function).
    num_players (int): The number of players.    
    ...
    if len(totalplayer) >= 1:  # If there are remaining players to consider
        cur_player = totalplayer[0]  # Select the current target
        temp = 0
        totalplayer = totalplayer[1:]  # Remove the current target from the list of remaining players
        for strategy in range(strategies[cur_player]):  # Iterate over all possible strategies for the current target
        ...
        return temp  # Return the equilibrium index after evaluating all strategies for the current target
        ...
        for strategy in range(strategies[playerno]):
            temp = strategyarr[:]
            temp.insert(playerno, strategy)
            indi = sel_index(playerno, temp, multiplier, num_players)
            util = gamedata[indi]
            if( max_payoff < util):
                max_payoff = util
                max_index = strategy
            else:
                other_payoffs.append(util)
...
def find_weakly_dominant_eq(gamedata, playerno, totalplayer, topplayer, strategies, 
    ...
    """
    Finds weakly dominant equilibrium strategy for a given target.
    ...
    if len(totalplayer) >= 1:
        ...
        for strategy in range(strategies[playerno]):
            ...
            if( max_payoff < util):
                max_payoff = util
                indices.clear()
                indices.append(strategy)
            elif max_payoff == util:
                indices.append(strategy)
        if eqindex[0] == -1:
            eqindex = indices
        else:
            eqindex = intersect(eqindex, indices)
...
def psne_gen(num_players, strategy, util_matrix):
    ...
    for j in allpermut:
        ...
        for target in range(num_players):
            for strat in range(strategy[target]):
                arr = j[:]
                arr[target] = strat 
                if util_matrix[tuple(j)][target] < util_matrix[tuple(arr)][target]:
                    is_nash =False
                    break
            if not is_nash:
                break
...
def msne_gen(num_players, strategy, util_matrix):
    ...
    ##########     TASK 4         ###############
    util000 = util_matrix[0][0][0]
    util001 = util_matrix[0][0][1]
    util010 = util_matrix[0][1][0]
    util011 = util_matrix[0][1][1]
    util100 = util_matrix[1][0][0]
    util101 = util_matrix[1][0][1]
    util110 = util_matrix[1][1][0]
    util111 = util_matrix[1][1][1]
    p = (util110 - util100)/(util000 - util100 + util110 - util010)
    q = (util111 - util101)/(util001 - util101 + util111 - util011)
