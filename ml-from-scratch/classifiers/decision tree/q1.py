...
def information_gain(examples, classes, attr):
    ...
    entropy_child= prob * temp

    information_gain_value = entropy_par - np.sum(entropy_child)
...
class DecisionTree:
    ...
    def __build_tree__(self, features, classes, attributes, depth=0):
        ...
        if (len(attributes) == 0):
            ...
            best_attr = max(attributes , key = lambda a : information_gain(features,classes,a))

            # Get unique values of the best attribute for splitting.
            
            unique = np.unique(features[:,best_attr])
            ...
            rem = [a for a in attributes if a != best_attr]

            # Split the dataset based on the unique values of the best attribute.
            for un in unique:
                index = np.where(features[:,best_attr] == un)[0]
                new_fet = features[index]
                new_class = classes[index]

                if(len(np.unique(classes)) == 1):
                    children[un] = DecisionNode(classes[0], best_attr, children)

                else:
                    children[un] = self.__build_tree__(new_fet , new_class , rem , depth=depth+1)

        
            # Determine the default prediction
            ind = np.argmax(np.unique(classes , return_counts=True)[1])
            default_for_empty = classes[ind]
