...
class SKLearnDecisionTree:
    ...
    def train_model(self, criterion="entropy"):
        ...
        err1 = metrics.accuracy_score(pred_train , self.Y_train)
        err2 = metrics.accuracy_score(pred_test , self.Y_test)

        results = {}
        results["train_accuracy"] = err1
        results["test_accuracy"] = err2
    ...
    def train_model_regularised(self, criterion="entropy"):
        ...
        for i in range(1,11):
            for j in range(3,31,3):
                for k in range(1,11):
                    clf = DecisionTreeClassifier(random_state=42,criterion=criterion , max_depth=i , min_samples_split=j , min_samples_leaf=k)
                    clf.fit(self.X_train,self.Y_train)
                    pred_train = clf.predict(self.X_train)
                    pred_test = clf.predict(self.X_test)
                    err1 = metrics.accuracy_score(pred_train , self.Y_train)
                    err2 = metrics.accuracy_score(pred_test , self.Y_test)
                    train_accuracies.append(err1)
                    test_accuracies.append(err2)
