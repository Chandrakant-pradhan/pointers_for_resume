...
class Perceptron():
    ...
    def fit(self, x, y, plot_flag, lr = 0.001, epochs = 100):
        ...
        for epoch in range(epochs):
            ...
            # TODO calculate y_pred directly using x and w. Do not use the predict() function here, that is only for 
            
            y_pred = np.sign(np.matmul(x, self.weights))

            #y_pred[y_pred == 0] = -1
            delta_index = np.nonzero(y_pred-y)[0]
            # TODO perform the weight update
            
            self.weights += lr*np.matmul(y[delta_index],x[delta_index])
...
def accuracy(y_test, y_pred):
   ...
   #TODO calculate the accuracy
    err = y_test - y_pred
    correct = np.count_nonzero(err==0)
    accr = correct/len(y_test)
    return accr
