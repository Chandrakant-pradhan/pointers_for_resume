...
def log_loss(y, y_dash):
    ...
    loss = -1*(1-y)*np.log(1-y_dash) + -1*y*np.log(y_dash)
...
def cost_logreg(X, y, w, w_0):
    ...
    y_pred = logistic(np.dot(X,w) + w_0)

    cost = np.mean(log_loss(y , y_pred))
...
def grad_logreg(X, y, w, w_0):
    ...
    # for i in range(n):
    #     for j in range(m):
    #         grad_w[i] += (logistic(np.dot(X[j],w) + w_0) - y[j])*X[j][i]
    #     grad_w[i] /= m

    grad_w = np.dot(np.transpose(X),logistic(np.dot(X,w) + w_0)-y) 
    grad_w /= m

    
    # for i in range(m):
    #         grad_w_0 += (logistic(np.dot(X[i],w) + w_0) - y[i])

    # grad_w_0 /= m

    grad_w_0 = np.mean(logistic(np.dot(X,w) + w_0) - y)
