...
def w_closed_form(X, Y):
    ...
    x_trans = torch.t(X)
    mul1 = torch.linalg.matmul(x_trans , X)
    mul2 = torch.linalg.inv(mul1)
    mul3 = torch.linalg.matmul(x_trans , Y)

    w_closed = torch.linalg.matmul(mul2 , mul3)

    w_closed = torch.round(w_closed , decimals = 4)
...
def l2_loss(X, Y, w):
    ...
    #     yi = torch.linalg.matmul(w_trans , X[i])
    #     temp = (yi[0] - Y[i]).clone().detach().item()
    #     loss += temp ** 2 

    # y_pred =torch.linalg.matmul(w_trans , X)

    my_matrix = torch.linalg.matmul(X,w)
    my_matrix = my_matrix - Y

    my_matrix2 = torch.t(my_matrix)

    loss = torch.linalg.matmul(my_matrix2,my_matrix)
...
def l2_loss_derivative(X, Y, w):
    ...
    mul2 = torch.linalg.matmul(mul1 , w) 
...
def train_model(X_train, Y_train, X_test, Y_test, w, eta):
    ...
    while( abs(old_loss - l2_loss(X_train, Y_train, w)) > epsilon ):  
        ...
        delta = l2_loss_derivative(X_train,Y_train,w)
        w = w - eta * delta
        # print(abs(old_loss - l2_loss(X_train, Y_train, w)))
