...
def create_weights(data_dictionary, lambda_val):
    ...
    my_x  = data_dictionary['X_train']
    my_y = data_dictionary['Y_train']
    '''w = inv((XtX+λI)) * Xty '''
    my_x_trans = np.transpose(my_x)
    
    term1 = (np.matmul(my_x_trans , my_x) + lambda_val*np.identity(len(my_x[0]))) # Xt X + lambda I

    term2 = np.linalg.inv(term1) # inv of previous term

    term3 = np.matmul(my_x_trans , my_y) # Xt Y

    weights = np.matmul(term2 , term3)  # inv(Xt X + lambda I) * Xt Y
...
def generate_test_error(data_dictionary, weights):
    ...
    my_x  = data_dictionary['X_test']
    my_y = data_dictionary['Y_test']
    
    
    size = len(my_y)

    term1 = np.matmul(my_x,weights) - my_y   # (Xw - y)
    term1_trans = np.transpose(term1) #(Xw - y)t

    term2 = np.matmul(term1_trans , term1)

    test_error = term2[0][0]/size
