...
def pulp_solver(A_matrix: numpy.array, c: numpy.array, b: numpy.array) -> (numpy.array, float):
    ...
    nums = [pulp.LpVariable(f'x{i}', lowBound=0) for i in range(len(c))]

    
    objective = pulp.lpSum(c[i] * nums[i] for i in range(len(c)))
    prob += objective

    
    for i in range(len(A_matrix)):
        constraint = pulp.lpSum(A_matrix[i][j] * nums[j] for j in range(len(c))) <= b[i]
        prob += constraint
    ...
    x = numpy.array([pulp.value(num) for num in nums])
